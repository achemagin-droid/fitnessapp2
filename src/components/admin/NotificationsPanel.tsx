...
import { Bell, Send, Clock, CheckCircle2, AlertCircle, MessageCircle, Mail } from 'lucide-react';
...
export default function NotificationsPanel() {
  ...
  // Find upcoming sessions (within next 24h) that have confirmed bookings
  const upcomingNotifications = useMemo(() => {
    ...
    sessions.forEach((session) => {
      ...
      const twoHoursBefore = addHours(startTime, -2);
      
      // Show sessions that are upcoming and within notification window
      if (isAfter(startTime, now) && isBefore(startTime, addHours(now, 48))) {
        ...
        sessionBookings.forEach((booking) => {
          ...
          const isReminderTime = isAfter(now, twoHoursBefore) || isBefore(now, twoHoursBefore);
