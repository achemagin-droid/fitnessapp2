...
import { deleteAdminClient, getAdminClients, AdminClient } from '../../api';

export default function ClientCRM({ token }: { token?: string }) {
  ...
  const [remoteClients, setRemoteClients] = useState<AdminClient[]>([]);
  const { useMockData } = useStore();
  useEffect(() => { if (!useMockData) getAdminClients(token).then(result => setRemoteClients(result.clients)).catch(() => setRemoteClients([])); }, [useMockData, token]);
  const listClients = useMockData ? clients : remoteClients;
  ...
  return (
    <div className="animate-fade-in">
      ...
      <button onClick={() => setShowAll(!showAll)} className="mb-4 px-4 py-2 bg-gray-900 text-white rounded-xl text-sm">{showAll ? 'Скрыть список' : `Показать всех клиентов (${listClients.length})`}</button>
      {showAll && <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm mb-6 space-y-2">{listClients.map(item => <div key={item.id} className="flex items-center justify-between border-b last:border-0 py-2"><div><b>{item.first_name} {item.last_name}</b><p className="text-xs text-gray-500">{item.phone} · {item.email || 'email нет'}</p></div><button onClick={async () => { if (!window.confirm('Удалить клиента и связанные записи?')) return; if (useMockData) deleteClient(item.id); else { await deleteAdminClient(token, item.id); setRemoteClients(prev => prev.filter(client => client.id !== item.id)); } if (selectedClientId === item.id) setSelectedClientId(null); }} className="p-2 text-red-500" title="Удалить клиента"><Trash2 className="w-4 h-4" /></button></div>)}</div>}
